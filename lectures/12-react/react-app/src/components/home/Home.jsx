...
const Home = () => {
    ...
    useEffect( () => {
        async function fetchData() {
            const games = await fetchGames(filter); //browser, pc, all
    ...
    return <>
        <div style={{display: "flex", justifyContent: "start", alignItems: "center"}}>
            <button onClick={() => {setFilter('pc')}}>Show PC games</button>
            <button onClick={() => {setFilter('browser')}}>Show browser games</button>
        </div>
            <ul style={{listStyle: "none", display: "flex", gap: 50, flexWrap: "wrap"}}>
        {games.map((game) => {
